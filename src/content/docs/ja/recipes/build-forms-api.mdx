...
description: JavaScriptを使用して、フォームの送信データをAPIルートに送信する方法を学びます。
type: recipe
i18nReady: false
---
...
HTMLフォームでは、ブラウザがページをリフレッシュしたり、新しいページに移動したりします。代わりにフォームデータをAPIエンドポイントに送信するには、JavaScriptを使用してフォームの送信をインターセプトする必要があります。

このレシピでは、フォームデータをAPIエンドポイントに送信し、そのデータを処理する方法を説明します。

## 前提条件
- [オンデマンドレンダリング用のアダプター](/ja/guides/on-demand-rendering/)が設定されたプロジェクト
- [UIフレームワークのインテグレーション](/ja/guides/framework-components/)がインストールされていること

## レシピ

<Steps>
1. フォームデータを受け取るための `POST` APIエンドポイントを `/api/feedback` に作成します。`request.formData()` を使用して処理します。フォームの値は使用する前に必ずバリデーションしてください。

    この例では、メッセージを含むJSONオブジェクトをクライアントに返します。
    ...
    export const POST: APIRoute = async ({ request }) => {
      ...
      // データをバリデーションする - おそらくこれ以上のことが必要になります
...
2. UIフレームワークを使用してフォームコンポーネントを作成します。各入力には、その入力の値を説明する `name` 属性が必要です。
...
3. 送信イベントを受け入れる関数を作成し、それをフォームの `submit` ハンドラとして渡します。

    関数内では：
    - イベントで `preventDefault()` を呼び出して、ブラウザのデフォルトの送信プロセスを上書きします。
    - `FormData` オブジェクトを作成し、`fetch()` を使用してエンドポイントに `POST` リクエストで送信します。
...
4. `<FeedbackForm />`コンポーネントをページにインポートして含めます。`client:*`ディレクティブを使用して、フォームのロジックが必要なときにハイドレートされるようにしてください。
